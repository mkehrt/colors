// Based on code from http://rsbweb.nih.gov/ij/plugins/download/Color_Space_Converter.java.
//
// The original authors are Duane Schwartzwald and Harry Parker.   Used and
// distributed with permission.


struct Color {
    double* RGBtoHSB(R, G, B, hsb);
...
struct ColorSpaceConverter {
   ...
   */
  double[3] D50 = {96.4212, 100.0, 82.5188};
  double[3] D55 = {95.6797, 100.0, 92.1481};
  double[3] D65 = {95.0429, 100.0, 108.8900};
  double[3] D75 = {94.9722, 100.0, 122.6394};
  double[3] whitePoint = D65;

  /**
   * reference white in xyY coordinates
   */
  double chromaD50[3] = {0.3457, 0.3585, 100.0};
  double chromaD55[3] = {0.3324, 0.3474, 100.0};
  double chromaD65[3] = {0.3127, 0.3290, 100.0};
  double chromaD75[3] = {0.2990, 0.3149, 100.0};
  double chromaWhitePoint[3] = chromaD65;

  /**
   * sRGB to XYZ conversion matrix
   */
  double[3][3] M   = {{0.4124, 0.3576,  0.1805},
   ...
   */
  double[3][3] Mi  = {{ 3.2406, -1.5372, -0.4986},
   ...
   * @return RGB values
   */
  int[3] HSBtoRGB(double H, double S, double B) {
   ...
   * @return RGB values
   */
  int[3] LABtoRGB(double L, double a, double b) {
   ...
   * @return XYZ values
   */
  double[3] LABtoXYZ(double L, double a, double b) {
    double[3] result = new double[3];
   ...
   * @return XYZ values
   */
  double[3] LABtoXYZ(double[3] Lab) {
   ...
   */
  double[3] RGBtoHSB(int R, int G, int B) {
    double[3] result = new double[3];
    float[3] hsb = new float[3];
   ...
   * @return Lab values
   */
  double[3] RGBtoLAB(int R, int G, int B) {
   ...
   */
  double[3] RGBtoXYZ(int R, int G, int B) {
    double[3] result = new double[3];
   ...
   * @return XYZ values
   */
  double[3] xyYtoXYZ(double x, double y, double Y) {
    double[3] result = new double[3];
    if (y == 0) {
      result[0] = 0;
      result[1] = 0;
   ...
   * @return XYZ values
   */
  double[3] xyYtoXYZ(double[3] xyY) {
   ...
   * @return Lab values
   */
  double[3] XYZtoLAB(double X, double Y, double Z) {
   ...
   * @return Lab values
   */
  double[3] XYZtoLAB(double[3] XYZ) {
   ...
   */
  int[3] XYZtoRGB(double X, double Y, double Z) {
    int[3] result = new int[3];
   ...
   * @return xyY values
   */
  double[3] XYZtoxyY(double X, double Y, double Z) {
    double[3] result = new double[3];
    if ((X + Y + Z) == 0) {
   ...
   * @return xyY values
   */
  double[3] XYZtoxyY(double[3] XYZ) {
